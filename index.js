/* 
This is the server side node js code for the DDI-TimeOff-Calendar
url: TBD
The code below is meant to retrieve data from the form on the DDI-TimeOff-Calendar website.
It then processes that data and sends it to the tsheets website that DDI uses for time keeping
It also updates a google calendar to see a visual of the time off requests on the website itself
Finally it sends email confirmation to the managers and the user
...
try {	
	...
	let to = 'kianmoriarty@delzerbiz.com';
	let content = `There was an error with the DDI_Time_Off_Website\nERROR: ${err}`;
...
function main(){
	...
	employeeNames(data => {
		console.log(`Starting Employees Synced`); // print statement that employees were properly synced
	});

	// requirements
	const express = require('express'); // host requirement
	const app = express();
	require('dotenv').config(); // .env requirement
	...
	// Establish api settings
	app.use(express.json({ limit: '100mb' }));

	// API get request setup
	// this api is run at the opening of the webpage, it sends the employee list to the client webpage
	...
	// this api is run when the submit button is pushed
	// it recieves the form data, sends the request to tsheets, sends a request to the google calendar, and sends an email to the time appropriate managers as well as the user
